...
function setAnalyzeEnabled(on){ analyzeBtn.disabled = !on; }
...
function setABGains(j){
  // Use input metrics as reference; otherwise -14 LUFS
  const refI = j?.metrics?.input?.lufs_integrated ?? -14.0;

  const srcI = j?.metrics?.input?.lufs_integrated;
  const clubI = j?.metrics?.club?.lufs_integrated;
  const strI  = j?.metrics?.streaming?.lufs_integrated;
  const preI  = j?.metrics?.unlimited?.lufs_integrated;
  const cusI  = j?.metrics?.custom?.lufs_integrated;
...
function handleFile(f){
  if(!f) return setAnalyzeEnabled(false);
  selectedFile = f;
  if(selectedBlobUrl) URL.revokeObjectURL(selectedBlobUrl);
  selectedBlobUrl = URL.createObjectURL(f);
  loadIntoWave(selectedBlobUrl,'Original upload');
