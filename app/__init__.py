# app.py
import os, re, io, json, uuid, zipfile, hashlib, threading, subprocess
...
def new_session_dir() -> Tuple[str, Path]:
    ...
    d = Path(app.config["UPLOAD_FOLDER"]) / sid
    d.mkdir(parents=True, exist_ok=True)
    return sid, d

def progress_path(session: str) -> Path:
    return Path(app.config["UPLOAD_FOLDER"]) / session / "progress.json"

def write_json(path: Path, data: dict): path.write_text(json.dumps(data))
def read_json(path: Path, default: dict) -> dict: return json.loads(path.read_text()) if path.exists() else default

def base_progress() -> dict:
    return {
        "percent": 0, "phase": "starting", "message": "Starting…", "done": False,
        "preset": "club",
...
def extract_json_block(text: str) -> Dict[str, Any]:
    start = text.rfind("{")
    if start == -1: raise ValueError("No JSON block found")
    depth = 0
...
def measure_loudnorm_json(path: Path, I=-14.0, TP=-1.0, LRA=11.0) -> Dict[str, float]:
    cmd = ["ffmpeg","-hide_banner","-y","-i",str(path),
           "-filter:a",f"loudnorm=I={I}:TP={TP}:LRA={LRA}:print_format=json","-f","null","-"]
    code, out, err = run(cmd, timeout=900)
    if code != 0: raise RuntimeError(f"loudnorm measure failed: {err}")
    data = extract_json_block(err)
    outd = {}
    for k,v in data.items():
        try: outd[k] = float(v)
        except Exception: outd[k] = v
    return outd

def measure_peak_dbfs(path: Path) -> float:
    code, out, err = run(["ffmpeg","-hide_banner","-y","-i",str(path),"-filter:a","volumedetect","-f","null","-"], timeout=600)
    if code != 0: raise RuntimeError(f"volumedetect failed: {err}")
    m = re.search(r"max_volume:\s*(-?\d+(?:\.\d+)?)\s*dB", err)
    if not m: raise RuntimeError("Could not parse max_volume")
    return float(m.group(1))

def ebur128_timeline(path: Path) -> dict:
    cmd = ["ffmpeg","-hide_banner","-i",str(path),"-filter_complex","ebur128=peak=true:framelog=verbose","-f","null","-"]
    code, out, err = run(cmd, timeout=1800)
    if code != 0: return {"sec": [], "short_term": [], "tp_flags": []}
    sec, st, tp = [], [], []
    for line in err.splitlines():
        ...
        if mt and ms:
            ...
            tp.append(1 if (mp and float(mp.group(1)) > -1.0) else 0)  # hotspot if TP > -1 dBTP
    return {"sec": sec, "short_term": st, "tp_flags": tp}

# ---------- Processing stages ----------
def loudnorm_two_pass(in_path: Path, out_path: Path, I: float, TP: float, LRA: float, sr: int, bits: int, dither: str | None, smart_limiter: bool):
    # pass 1 analyze
    ...
    # pass 2 apply
    cmd = ["ffmpeg","-hide_banner","-y","-i",str(in_path),
           *limiter_pre,
           "-ar", str(sr), *codec,
           "-filter:a",
           ("loudnorm="
    ...
    if code != 0: raise RuntimeError(f"loudnorm pass-2 failed: {err}")
    return m  # input stats

def normalize_peak_to(in_path: Path, out_path: Path, target_dbfs: float, sr=48000, bits=24, dither: str | None = None):
    ...
    if bits == 16 and dither:
        dither_args = ["-af", f"aresample={sr}:dither_method={dither}"]
    code, out, err = run(["ffmpeg","-hide_banner","-y","-i",str(in_path),
                          "-ar", str(sr), *codec, "-filter:a", f"volume={gain}dB",
                          *dither_args, str(out_path)], timeout=1200)
    if code != 0: raise RuntimeError(f"peak normalize failed: {err}")
    return peak

def trim_and_pad(in_path: Path, out_path: Path, trim: bool, pad_ms: int, sr: int, bits: int):
    filters = []
    if trim:
        filters.append("silenceremove=start_periods=1:start_threshold=-50dB:start_silence=0.2:stop_periods=1:stop_threshold=-50dB:stop_silence=0.3")
    ...
    codec = ["-c:a","pcm_s24le"] if bits==24 else ["-c:a","pcm_s16le"]
    code, out, err = run(["ffmpeg","-hide_banner","-y","-i",str(in_path), "-ar", str(sr), *codec, "-af", af, str(out_path)], timeout=1200)
    if code != 0: raise RuntimeError(f"trim/pad failed: {err}")

def ffprobe_info(path: Path) -> dict:
    code, out, err = run(["ffprobe","-hide_banner","-print_format","json","-show_format","-show_streams",str(path)], timeout=60)
    if code != 0: raise RuntimeError(f"ffprobe failed: {err}")
    return json.loads(out)

def validate_upload(path: Path, max_minutes: int = 20):
    info = ffprobe_info(path)
    dur = float(info.get("format", {}).get("duration", "0") or 0)
    if dur <= 0: raise RuntimeError("Unrecognized or zero-length audio")
    if dur > max_minutes * 60: raise RuntimeError(f"Audio too long (> {max_minutes} minutes)")

def recommend_preset(input_I: float, input_LRA: float, input_TP: float) -> str:
    if input_I <= -12 and input_LRA <= 8:  return "spotify"
    if input_I <= -14:                      return "apple"
    if input_I > -10:                       return "club"
    return "youtube"

def mix_stems_to_wav(stems: Dict[str, Path], gains: Dict[str,float], out_path: Path, sr=48000, bits=24):
    inputs, vols, labels = [], [], []
    ...
    codec = ["-c:a","pcm_s24le"] if bits==24 else ["-c:a","pcm_s16le"]
    cmd = ["ffmpeg","-hide_banner","-y", *inputs, "-filter_complex", ";".join(vols + [amix]), "-map","[aout]", "-ar", str(sr), *codec, str(out_path)]
    code, out, err = run(cmd, timeout=1800)
...
def start():
    ...
    if not allowed_file(f.filename): return jsonify({"error":"Unsupported type"}), 400
    ...
    update_progress(session, {
        "percent": 1, "phase":"queued","message":"Queued",
        "preset": params["preset"],
        "options": {"trim": params["trim"], "pad_ms": params["pad_ms"], "smart_limiter": params["smart_limiter"], "bits": params["bits"], "dither": params["dither"]}
    })

    t = threading.Thread(target=run_pipeline, args=(session, src, params, stems if stems else None, gains if gains else None), daemon=True)
    t.start()
...
def download_file(session: str, filename: str):
    d = Path(app.config["UPLOAD_FOLDER"]) / session
    if not d.exists(): return "Not found", 404
